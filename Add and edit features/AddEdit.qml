...
App {
    ...
    Map {
        ...
        Column {
            ...
            Text{
                text: isMobile ? "Tap on the map to add a feature" : "Click on the map to add a feature";
            }

            Text{
                text: isMobile ? "Tap on a feature to edit its attributes" : "Click on a feature to edit its attributes";
            }
        }
        ...
        onMouseClicked: {
            ...
            for ( var i = 0; i < features.length; i++ ) {
                dispTable(features[i]);
            }
            //ADD FEATURE
            if(features.length === 0){
                addPoint(mouse.mapX, mouse.mapY, mouse.mapPoint.spatialReference);
                featureAdded = true;
            }
            else{
                featureAdded = false;
