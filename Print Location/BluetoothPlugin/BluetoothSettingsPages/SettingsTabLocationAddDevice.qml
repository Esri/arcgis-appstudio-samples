...
SettingsTab {
    ...
    Item {
        ...
        readonly property string kDescriptionBluetooth: qsTr("All printers that are paired to the device will be displayed in this list. If your printer is missing please check that Bluetooth is enabled and that the printer is paired.")
        readonly property bool scanSerialPort: bluetoothSettings.discoverSerialPort
                        ...
                        if (_item.initialized && !updating) {
                            if (checked) {
                                if (!iOS || _item.scanSerialPort || Permission.serviceStatus(Permission.BluetoothService) === Permission.ServiceStatusPoweredOn) {
                                    ...
                                    bluetoothSourceManager.discoveryAgentError("")
                                    checked = false;
                                }
                            } else {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        var deviceName = bluetoothSettings.createExternalReceiverSettings(name, device.toJson());
