...
define([
  ...
  "dojo/touch",
  "dojox/gesture/tap"
], function(declare, domConstruct, lang, domAttr, template, _WidgetBase,
  _TemplatedMixin, _WidgetsInTemplateMixin, nls, ItemDetails, query,
  domClass, on, Deferred, number, topic, domStyle, domGeom, touch, tap) {

  declare("ItemGallery", [_WidgetBase, _TemplatedMixin,
     ...
     */
    postCreate: function() {
      domConstruct.place(this.galleryView, query(
        ".esriCTGalleryContent")[0]);
      ...
      if (dojo.configData.values.defaultLayout.toLowerCase() ===
        "list") {
        dojo.gridView = false;
        domClass.replace(query(".icon-header")[0], "icon-grid",
          "icon-list");
      } else {
        dojo.gridView = true;
        domClass.replace(query(".icon-header")[0], "icon-list",
          "icon-grid");
      }
      // Show more items from the group on click of 'Show more' button
      this.own(on(this.galleryNext, "click", lang.hitch(this,
        function() {
          var defObj;
          topic.publish("showProgressIndicator");
          defObj = new Deferred();
          topic.publish("queryGroupItem", null, null, null,
            defObj, dojo.nextQuery);
      ...
      this.own(on(query(".esriCTMenuTabLeft")[0], "click", lang.hitch(
        this,
        function() {
          if (query(".esriCTitemDetails")[0]) {
            dojo.destroy(query(".esriCTitemDetails")[0]);
            domClass.remove(query(".esriCTGalleryContent")[0],
              "displayNoneAll");
            domClass.remove(query(".esriCTApplicationIcon")[0],
              "esriCTCursorPointer");
            domClass.remove(query(".esriCTMenuTabLeft")[0],
              "esriCTCursorPointer");
          }
          if (query(".esriCTInnerRightPanelDetails")[0] && (!
              query(".esriCTNoResults")[0])) {
            domClass.replace(query(".esriCTMenuTabRight")[0],
              "displayBlockAll", "displayNoneAll");
            domClass.add(query(".esriCTInnerRightPanelDetails")[
              0], "displayNoneAll");
            domClass.remove(query(".esriCTGalleryContent")[0],
              "displayNoneAll");
            domClass.remove(query(".esriCTInnerRightPanel")[0],
              "displayNoneAll");
            domClass.remove(query(".esriCTApplicationIcon")[0],
              "esriCTCursorPointer");
            domClass.remove(query(".esriCTMenuTabLeft")[0],
      ...
      on(window, "resize", lang.hitch(this, function() {
        var leftPanelDescHeight, containerHeight,
          innerLeftPanelHeight, tagContainerHeight, descHeight,
          descContainerHeight,
          listSortMenu, i, j;
        if (domClass.contains(query(
              ".esriCTInnerLeftPanelBottom")[0],
            "esriCTInnerLeftPanelBottomShift")) {
          innerLeftPanelHeight = dojo.window.getBox().h + "px";
          domStyle.set(query(".esriCTInnerLeftPanelBottom")[0],
            "height", innerLeftPanelHeight);
        }
        if (dojo.configData.groupDescription) {
          if (domStyle.get(query(".esriCTSignInIcon")[0],
              "display") !== "none") {
            descHeight = window.innerHeight / 5;
            leftPanelDescHeight = window.innerHeight - (domGeom
              .position(query(".esriCTMenuTab")[0]).h +
              domGeom.position(query(
                ".esriCTInnerLeftPanelBottom")[0]).h -
              descHeight) + "px";
            domStyle.set(query(".esriCTGroupDesc")[0], "height",
              leftPanelDescHeight);
          }
        }
        containerHeight = (window.innerHeight - domGeom.position(
          query(".esriCTMenuTab")[0]).h - 20) + "px";
        domStyle.set(query(".esriCTInnerRightPanel")[0],
        ...
        if (dojo.configData.groupDescription) {
          if (domStyle.get(query(".esriCTSignInIcon")[0],
              "display") === "none") {
            domClass.remove(query(".esriCTGroupDesc")[0],
              "esriCTLeftTextReadLess");
            domStyle.set(query(".esriCTExpand")[0], "display",
              "none");
            descContainerHeight = window.innerHeight - (domGeom
              .position(query(".esriCTGalleryNameSample")[0])
              .h + 100) + "px";
            domStyle.set(query(".esriCTGroupDesc")[0], "height",
              descContainerHeight);
          } else {
            domStyle.set(query(".esriCTGroupDesc")[0], "height",
              "");
            if (query(query(".esriCTGroupDesc")[0]).text().length >
              400) {
              domClass.add(query(".esriCTGroupDesc")[0],
                "esriCTLeftTextReadLess");
              domAttr.set(query(".esriCTExpand")[0],
                "innerHTML", nls.expandGroupDescText);
            }
            domStyle.set(query(".esriCTExpand")[0], "display",
              "block");
          }
        ...
        if (dojo.configData.values.showTagCloud) {
          if (domClass.contains(query(".esriCTSignIn")[0],
              "displayNone")) {
            tagContainerHeight = window.innerHeight - (domGeom.position(
                query(".sortByLabelMbl")[0]).h + domGeom.position(
                query(".esriCTCategoriesHeader")[0]).h + 40) +
              "px";
            domStyle.set(query(".esriCTPadding")[0], "height",
              tagContainerHeight);
          } else {
            tagContainerHeight = window.innerHeight - (domGeom.position(
                  query(".esriCTCategoriesHeader")[0]).h +
                domGeom.position(query(".esriCTMenuTab")[0]).h +
                domGeom.position(query(
                  ".esriCTInnerLeftPanelTop")[0]).h + 30) +
              "px";
            domStyle.set(query(".esriCTPadding")[0], "height",
              tagContainerHeight);
          }
        ...
        if (query(".esriCTListSelected")[0]) {
          setTimeout(lang.hitch(this, function() {
            if (query(".esriCTSortMenuListSelected")[0] &&
              domAttr.get(query(
                  ".esriCTSortMenuListSelected")[0].parentElement
                .parentElement, "sortValue") !== dojo.sortBy &&
              domAttr.get(query(
                  ".esriCTSortMenuListSelected")[0].parentElement,
                "sortOrder") !== dojo.sortOrder) {
              listSortMenu = query(
                  ".esriCTSortMenuListSelected")[0].parentElement
                .parentElement.parentElement;
              for (i = 0; i < listSortMenu.children.length; i++) {
                if (domAttr.get(listSortMenu.children[i],
                    "sortValue") === dojo.sortBy) {
                  for (j = 0; j < listSortMenu.children[i]
                    .children.length; j++) {
                    if (domAttr.get(listSortMenu.children[
                        i].children[j], "sortOrder") ===
                      dojo.sortOrder) {
                      domClass.remove(query(
                          ".esriCTSortMenuListSelected"
                        )[0],
                        "esriCTSortMenuListSelected");
                      domClass.add(listSortMenu.children[
                          i].children[j].children[0],
                        "esriCTSortMenuListSelected");
                      break;
                    }
                  }
      ...
      this.own(on(this.backToGalleryBtn, "click", lang.hitch(this,
        this._backToGalleryView)));
      //handler to display gallery view
      topic.subscribe("backToGalleryView", lang.hitch(this, this._backToGalleryView));
      var panelHeight = (window.innerHeight - domGeom.position(query(
        ".esriCTMenuTab")[0]).h - 20) + "px";
      domStyle.set(query(".esriCTInnerRightPanel")[0], "height",
        panelHeight);
    },
    ...
    _backToGalleryView: function() {
      ...
      if (query(".esriCTInnerRightPanelDetails")[0] && (!query(
          ".esriCTNoResults")[0])) {
        domClass.replace(query(".esriCTMenuTabRight")[0],
          "displayBlockAll", "displayNoneAll");
        domClass.add(query(".esriCTInnerRightPanelDetails")[0],
          "displayNoneAll");
        domClass.remove(query(".esriCTGalleryContent")[0],
          "displayNoneAll");
        domClass.remove(query(".esriCTInnerRightPanel")[0],
    ...
    createItemPods: function(itemResults, clearContainerFlag) {
      ...
      if (query(".esriCTShowMoreResults")[0]) {
        if (itemResults.length < 100 || (itemResults.length === 100 &&
            dojo.groupItems.length === 100)) {
          domClass.replace(query(".esriCTShowMoreResults")[0],
            "displayNoneAll", "displayBlockAll");
        } else {
          domClass.replace(query(".esriCTShowMoreResults")[0],
            "displayBlockAll", "displayNoneAll");
        }
      ...
      for (i = 0; i < itemResults.length; i++) {
        if (!dojo.gridView) {
          ...
          domClass.replace(this.itemPodsList, "esriCTListView",
            "esriCTGridView");
          divPodParentList = domConstruct.create('div', {
            "class": "esriCTApplicationListBox"
          }, this.itemPodsList);
          this._createThumbnails(itemResults[i], divPodParentList);
          this._createItemOverviewPanel(itemResults[i],
            divPodParentList);
        } else {
          //add class to identify selected view mode
          domClass.replace(this.itemPodsList, "esriCTGridView",
            "esriCTListView");
          divPodParent = domConstruct.create('div', {
            "class": "esriCTApplicationBox"
          }, this.itemPodsList);
    ...
    _createGridItemOverview: function(itemResult, divPodParent) {
      var divItemTitleRight, divItemTitleText, divItemType,
        spanItemType, divItemWatchEye, spanItemWatchEyeText,
        divItemDetailsIcon, dataType;

      divItemTitleRight = domConstruct.create('div', {
        "class": "esriCTDivClear"
      }, divPodParent);
      divItemTitleText = domConstruct.create('div', {
        "class": "esriCTListAppTitle esriCTGridTitleContent esriCTCursorPointer esriCTHeaderBackgroundColorAsTextColor "
      }, divItemTitleRight);
      divItemDetailsIcon = domConstruct.create('div', {
        "class": "esriCTItemInfoIcon esriCTHeaderBackgroundColor"
      }, divItemTitleRight);
      domAttr.set(divItemTitleText, "innerHTML", (itemResult.title) ||
        (nls.showNullValue));
      domAttr.set(divItemTitleText, "title", (itemResult.title) || (
        nls.showNullValue));
      divItemType = domConstruct.create('div', {
        "class": "esriCTGridItemType"
      }, divItemTitleRight);
      spanItemType = domConstruct.create('div', {
        "class": "esriCTInnerGridItemType"
      }, divItemType);
      domAttr.set(spanItemType, "innerHTML", (itemResult.type) || (
        nls.showNullValue));
      ...
      if (dojo.configData.values.showViews) {
        divItemWatchEye = domConstruct.create('div', {
          "class": "esriCTEyeNumViews esriCTEyeNumViewsGrid"
        }, divItemType);
        domConstruct.create('span', {
          "class": "esriCTEyeIcon icon-eye"
        }, divItemWatchEye);
        spanItemWatchEyeText = domConstruct.create('span', {
          "class": "view"
        }, divItemWatchEye);
        domAttr.set(spanItemWatchEyeText, "innerHTML", (itemResult.numViews >=
            0) ? (number.format(parseInt(itemResult.numViews, 10))) :
          (nls.showNullValue));
      ...
      // Handle item title click in grid layout
      this.own(on(divItemDetailsIcon, "click", lang.hitch(this,
        function() {
          topic.publish("showProgressIndicator");
    ...
    _createThumbnails: function(itemResult, divPodParent) {
      var divThumbnail, divThumbnailImage, divTagContainer,
        divTagContent, dataType, thumbnailUrl, divItemSnippet,
        spanItemReadMore;

      if (!dojo.gridView) {
        divThumbnail = domConstruct.create('div', {
          "class": "esriCTImageContainerList"
        }, divPodParent);
      } else {
        divThumbnail = domConstruct.create('div', {
          "class": "esriCTImageContainer"
        }, divPodParent);
        divItemSnippet = domConstruct.create('div', {
          "class": "esriCTItemSnippet"
        }, divThumbnail);
        //create container to display snippet text of item on hovering of it
        if (itemResult.snippet) {
          spanItemReadMore = domConstruct.create("div", {
            "class": "esriCTItemSnippetText"
          }, divItemSnippet);
          domAttr.set(spanItemReadMore, "innerHTML", this._truncate(
            itemResult.snippet, 194));
        }
      }

      divThumbnailImage = domConstruct.create('div', {
        "class": "esriCTAppImage esriCTHeaderBackgroundColorAsBorder"
      }, divThumbnail);
      if (itemResult.thumbnailUrl) {
        if (dojo.configData.values.proxyUrl) {
          thumbnailUrl = dojo.configData.values.proxyUrl + "?" +
            itemResult.thumbnailUrl;
        } else {
          thumbnailUrl = itemResult.thumbnailUrl;
        }
        domStyle.set(divThumbnailImage, "background", 'url(' +
          thumbnailUrl + ') no-repeat center center');
      } else {
        domClass.add(divThumbnailImage, "esriCTNoThumbnailImage");
      }

      divTagContainer = domConstruct.create('div', {
        "class": "esriCTSharingTag"
      }, divThumbnailImage);
      divTagContent = domConstruct.create('div', {
        "class": "esriCTTag"
      }, divTagContainer);
    ...
    _showItemOverview: function(itemId, thumbnailUrl, itemResult, data) {
      var itemDetails, dataType, tokenString2, downloadPath,
        tokenString, itemUrl, defObject;

      if (data) {
        data.thumbnailUrl = thumbnailUrl;
        dataType = data.type.toLowerCase();
        if ((dataType === "map service") || (dataType === "web map") ||
          (dataType === "feature service") || (dataType ===
            "image service") || (dataType === "kml") || (dataType ===
            "wms") || (dataType === "vector tile service")) {
          if ((dataType === "web map") && dojo.configData.values.mapViewer
            .toLowerCase() === "arcgis") {
            topic.publish("hideProgressIndicator");
            window.open(dojo.configData.values.portalURL +
              '/home/webmap/viewer.html?webmap=' + itemId, "_self");
          } else {
          ...
          if (data.url) {
          ...
          } else if (data.itemType.toLowerCase() === "file" && data.type
            .toLowerCase() === "cityengine web scene") {
            dojo.downloadWindow.location = dojo.configData.values.portalURL +
              "/apps/CEWebViewer/viewer.html?3dWebScene=" + data.id;
          } else if (data.itemType.toLowerCase() === "text" && data.type
            .toLowerCase() === "web scene") {
            dojo.downloadWindow.location = dojo.configData.values.portalURL +
              "/home/webscene/viewer.html?webscene=" + data.id;
            ...
            downloadPath = dojo.configData.values.portalURL +
              "/sharing/content/items/" + itemId + "/data" +
              tokenString2;
            dojo.downloadWindow.location = downloadPath;
          } else if (dataType === "operation view" && data.itemType.toLowerCase() ===
            "text") {
            if (dojo.configData.values.token) {
            ...
            itemUrl = dojo.configData.values.portalURL +
              "/sharing/rest/content/items/" + data.id +
              "/data?f=json" + tokenString;
            defObject = new Deferred();
            topic.publish("queryItemInfo", itemUrl, defObject);
            defObject.then(lang.hitch(this, function(result) {
              if (dojo.configData.values.token) {
                tokenString = "?token=" + dojo.configData.values
                  .token;
              } else {
                tokenString = '';
              }
              if (result.desktopLayout) {
                downloadPath = dojo.configData.values.portalURL +
                  "/opsdashboard/OperationsDashboard.application?open=" +
                  data.id;
                dojo.downloadWindow.location = downloadPath;
              } else if (result.tabletLayout) {
                downloadPath = dojo.configData.values.portalURL +
                  "/apps/dashboard/index.html#/" + data.id;
                dojo.downloadWindow.location = downloadPath;
              }
            }));
          } else {
    ...
    _createItemOverviewPanel: function(itemResult, divPodParent) {
      var divContent, divTitle, divItemTitle, divItemTitleRight,
        divItemTitleText, divItemInfo, divItemType,
        divRatings, numberStars, i, imgRating, divItemWatchEye,
        spanItemWatchEyeText, divItemContent,
        divItemSnippet, spanItemReadMore, divEyeIcon, divItemViewIcon,
        divItemDetailsIcon, divItemBtnContainer, dataType;

      divContent = domConstruct.create('div', {
        "class": "esriCTListContent"
      }, divPodParent);
      divTitle = domConstruct.create('div', {
        "class": "esriCTAppListTitle"
      }, divContent);

      divItemTitle = domConstruct.create('div', {
        "class": "esriCTAppListTitleRight"
      }, divTitle);
      divItemTitleRight = domConstruct.create('div', {
        "class": "esriCTDivClear"
      }, divItemTitle);
      divItemTitleText = domConstruct.create('div', {
        "class": "esriCTListAppTitle  esriCTCursorPointer esriCTHeaderBackgroundColorAsTextColor"
      }, divItemTitleRight);
      domAttr.set(divItemTitleText, "innerHTML", (itemResult.title) ||
        (nls.showNullValue));
      ...
      divItemType = domConstruct.create('div', {
        "class": "esriCTListItemType"
      }, divItemInfo);
      domAttr.set(divItemType, "innerHTML", (itemResult.type) || (nls
        .showNullValue));
      // If showRatings flag is set to true in the configuration file, create rating stars
      if (dojo.configData.values.showRatings) {
        divRatings = domConstruct.create('div', {
          "class": "esriCTRatingsDiv"
        }, divItemInfo);
        ...
        for (i = 0; i < 5; i++) {
          ...
          if (i < numberStars) {
            domClass.add(imgRating,
              "icon-star esriCTRatingStarIcon esriCTRatingStarIconColor"
            );
          } else {
            domClass.add(imgRating,
              "icon-star-empty esriCTRatingStarIcon esriCTRatingStarIconColor"
            );
          }
        }
      }
      ...
      if (dojo.configData.values.showViews) {
        divItemWatchEye = domConstruct.create('div', {
          "class": "esriCTEyeNumViews esriCTEyeNumViewsList"
        }, divItemInfo);
        divEyeIcon = domConstruct.create('span', {
          "class": "esriCTEyeIcon icon-eye"
        }, divItemWatchEye);
        if (dojo.configData.values.showRatings) {
      ...
      divItemContent = domConstruct.create('div', {
        "class": "esriCTListAppContent"
      }, divContent);
      divItemSnippet = domConstruct.create('div', {
        "class": "esriCTAppHeadline esriCTBodyTextColor "
      }, divItemContent);
      if (itemResult.snippet) {
        spanItemReadMore = domConstruct.create('span', {},
          divItemSnippet);
      ...
      divItemBtnContainer = domConstruct.create('div', {
        "class": "esriCTItemBtnContainer"
      }, divItemContent);

      //create open/view button
      divItemViewIcon = domConstruct.create('div', {
        "class": "esriCTItemViewIcon esriCTHeaderBackgroundColor",
        "title": nls.title.viewBtnTitle
      }, divItemBtnContainer);
      ...
      //create info button
      divItemDetailsIcon = domConstruct.create('div', {
        "class": "esriCTItemDetailsIcon esriCTHeaderBackgroundColor",
        "title": nls.title.infoBtnTitle
      }, divItemBtnContainer);
    ...
    displayPanel: function(itemResult, itemFlag) {
      var numberOfComments, numberOfRatings, numberOfViews,
        itemReviewDetails, itemDescription, accessContainer,
        accessInfo, itemCommentDetails, itemViewDetails, itemText,
        containerHeight, dataArray, itemUrl, defObject, tokenString,
        dataType;
      ...
      itemUrl = dojo.configData.values.portalURL +
        "/sharing/content/items/" + itemResult.id + "?f=json" +
        tokenString;
      defObject = new Deferred();
      defObject.then(lang.hitch(this, function(data) {
        if (data) {
          ...
          dataArray = {};
          if (data.itemType === "file" && data.type.toLowerCase() !==
            "kml" && data.type.toLowerCase() !==
            "cityengine web scene") {
            domAttr.set(this.btnTryItNow, "title", nls.downloadButtonText);
            domClass.replace(this.btnTryItNow,
              "esriCTItemDownloadIcon", "esriCTItemViewIcon");
          } else if (data.type.toLowerCase() ===
            "operation view") {
            if (dojo.configData.values.token) {
              tokenString = "&token=" + dojo.configData.values.token;
            } else {
            ...
            domClass.replace(this.btnTryItNow,
              "esriCTItemViewIcon", "esriCTItemDownloadIcon");
          }
          ...
          if (data.type && data.type.toLowerCase() ===
            "vector tile service") {
            dataArray.url = dojo.configData.values.portalURL +
              "/sharing/content/items/" + data.id +
          ...
          if (itemFlag) {
            ...
            this._showItemOverview(itemResult.id, itemResult.thumbnailUrl,
              itemResult, dataArray);
          }
      ...
      // itemFlag indicates if item details page should be displayed or the item should be opened up
      if (itemFlag) {
        domClass.replace(query(".esriCTApplicationIcon")[0],
          "esriCTCursorPointer", "esriCTCursorDefault");
        domClass.replace(query(".esriCTMenuTabLeft")[0],
          "esriCTCursorPointer", "esriCTCursorDefault");
        domClass.replace(query(".esriCTMenuTabRight")[0],
          "displayNoneAll", "displayBlockAll");
        domClass.replace(query(".esriCTInnerRightPanel")[0],
          "displayNoneAll", "displayBlockAll");
        domClass.remove(query(".esriCTInnerRightPanelDetails")[0],
          "displayNoneAll");
        domConstruct.empty(this.detailsContent);
        domConstruct.empty(this.ratingsContainer);
        containerHeight = (window.innerHeight - domGeom.position(
          query(".esriCTMenuTab")[0]).h - 25) + "px";
        domStyle.set(query(".esriCTInnerRightPanelDetails")[0],
          "height", containerHeight);
        ...
        if (itemResult.thumbnailUrl) {
          ...
          domStyle.set(this.appThumbnail, "background", 'url(' +
            itemResult.thumbnailUrl + ') no-repeat center center');
        } else {
          domClass.add(this.appThumbnail, "esriCTNoThumbnailImage");
        }

        domAttr.set(this.applicationType, "innerHTML", (itemResult.type) ||
          (nls.showNullValue));
        ...
        if (dojo.configData.values.showViews) {
          ...
          numberOfViews = (itemResult.numViews) ? (number.format(
            parseInt(itemResult.numViews, 10))) : "0";
          ...
          itemText = "(" + itemCommentDetails + itemReviewDetails +
            itemViewDetails + ")";
        ...
        domConstruct.create('div', {
          ...
          "innerHTML": nls.appDesText
        }, this.detailsContent);
        itemDescription = domConstruct.create('div', {
          "class": "esriCTText esriCTReviewContainer esriCTBottomBorder esriCTBodyTextColor"
        }, this.detailsContent);
        ...
        if (dojo.configData.values.showLicenseInfo) {
          accessContainer = domConstruct.create('div', {
            "class": "esriCTReviewContainer esriCTBottomBorder esriCTBodyTextColor"
          }, this.detailsContent);
          domConstruct.create('div', {
            "class": "esriCTReviewHeader esriCTHeaderBackgroundColorAsTextColor",
            "innerHTML": nls.accessConstraintsText
          }, accessContainer);
          accessInfo = domConstruct.create('div', {
        ...
        dataType = itemResult.type.toLowerCase();
        this._btnTryItNowHandle = on(this.btnTryItNow, "click", lang.hitch(
          this,
          function() {
            if ((domAttr.get(this.btnTryItNow, "title") === nls.downloadButtonText) ||
              ((dataType !== "map service") && (dataType !==
                  "web map") && (dataType !== "feature service") &&
                (dataType !== "image service") && (dataType !==
                  "kml") && (dataType !== "wms") && (dataType !==
                  "vector tile service"))) {
            ...
            this._showTryItNowView(this.btnTryItNow, itemResult,
              dataArray);
          }));
        ...
        this._appThumbnailClickHandle = on(this.appThumbnail, "click",
          lang.hitch(this, function() {
            ...
            this._showTryItNowView(this.appThumbnail, itemResult,
              dataArray);
          }));
      }
    },
    ...
    _showTryItNowView: function(container, itemResult, dataArray) {
      ...
      this._showItemOverview(itemId, thumbnailUrl, itemResult,
        dataArray);
    },
    ...
    _createPropertiesContent: function(itemInfo, detailsContent) {
      var tagsContent, i, itemTags, sizeContent, itemSizeValue,
    ...
    _createItemDescription: function(itemResult, itemDescription) {
      ...
      domAttr.set(itemDescription, "innerHTML", itemResult.description ||
        "");
      domAttr.set(this.itemSubmittedBy, "innerHTML", (itemResult.owner) ||
        (nls.showNullValue));
    ...
    _createCommentsContainer: function(itemResult, detailsContent) {
      var reviewContainer = domConstruct.create('div', {
        "class": "esriCTReviewContainer esriCTBottomBorder"
      }, detailsContent);
      domConstruct.create('div', {
        "class": "esriCTReviewHeader",
        "innerHTML": nls.reviewText
      }, reviewContainer);
      itemResult.getComments().then(function(result) {
        ...
        if (result.length > 0) {
          for (i = 0; i < result.length; i++) {
            divReview = domConstruct.create('div', {
              "class": "esriCTReview"
            }, reviewContainer);
            divReviewHeader = domConstruct.create('div', {
              "class": "esriCTReviewBold"
            }, divReview);
            divReviewText = domConstruct.create('div', {
              "class": "esriCTReviewText esriCTBreakWord"
            }, divReview);
            domAttr.set(divReviewHeader, "innerHTML", (result[i].created) ?
              (result[i].created.toLocaleDateString()) : (nls.showNullValue)
            );
            try {
            ...
            domAttr.set(divReviewText, "innerHTML", (result[i].comment) ?
              comment : (nls.showNullValue));
          }
        } else {
          divReview = domConstruct.create('div', {
            "class": "esriCTDivClear"
